...
| D35 | **D26 rollback approved by Mike (2026-08-01) but not executable from this environment** — the destructive SQL was blocked by the tooling permission classifier (working as intended). The exact approved SQL is committed at `supabase/rollback/d26_remove_detour_drift.sql` for a one-paste run in the Supabase SQL Editor. | Keeps the approval + the exact commands on record (§11 item 7) without working around a safety control. |
