...
const ET_DATE = new Intl.DateTimeFormat('en-CA', { timeZone: ET, year: 'numeric', month: '2-digit', day: '2-digit' });
...
/** The UTC instants bounding TOMORROW as Hampton Roads lives it. */
export function etTomorrowWindow(now: Date): { startUtc: Date; endUtc: Date; dayLabel: string } {
  const dayLabel = ET_DATE.format(new Date(now.getTime() + 86400_000)); // en-CA => YYYY-MM-DD
  const [y, m, d] = dayLabel.split('-').map(Number);
  const naiveMidnightUtc = Date.UTC(y!, m! - 1, d!, 0, 0, 0);
  // Offset sampled at the target day handles the DST boundary correctly.
